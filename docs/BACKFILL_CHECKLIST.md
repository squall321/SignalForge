...
- [x] **pikabu** — 검색이 `?page=N` 으로 깊이 들어간다(페이지 안 링크에 page=100
      ...
      과거에 닿을 수 없었다.
- [ ] zdnet_kr — 탐색 URL 이 404 였다. 크롤러가 쓰는 실제 URL 로 재확인 필요
- [ ] 4pda — 탐색 시 403. 차단 우회 수단이 먼저 필요하다
- [ ] macrumors — `rel=next: /guide/samsung/2/` 가 있으나 크롤러는 피드를 쓴다.
      가이드 경로가 VOC 로 쓸 만한지 판단 필요
