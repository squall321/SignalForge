...
class _StubConn:
    ...
    async def fetch(self, sql, *args):
        if "AS total" in sql and "voc_defects" not in sql and "products" not in sql[:60]:
...
def _combo(cnt, indep=5, component="battery", symptom="drain", severity="degraded",
           tail=None):
    """indep = 독립 제보 소스 수, tail = 최근 TAIL_DAYS 건수(기본: 전부 최신)."""
    return {"product_code": "GX", "product_id": 1, "component": component,
            "symptom": symptom, "severity": severity, "cnt": cnt,
            "indep_sources": indep, "tail_cnt": cnt if tail is None else tail}
...
def test_detects_clear_spike():
    ...
    conn = _StubConn([_combo(60)], 1000, {"cnt": 10, "total": 1000})
    out = _run(conn)
    assert len(out) == 1
...
# ── 독립 제보가 부족하면 배제한다 (매체 신디케이션 차단) ──────────────
def test_insufficient_independent_sources_suppressed():
    # 독립 소스 1곳 — 뉴스 복제이거나 한 커뮤니티 반향. 급등해도 알리지 않아야 한다.
    conn = _StubConn([_combo(60, indep=1)], 1000, {"cnt": 10, "total": 1000})
    assert _run(conn) == []


def test_diverse_sources_pass():
    conn = _StubConn([_combo(60, indep=5)], 1000, {"cnt": 10, "total": 1000})
...
def test_below_min_count_not_evaluated():
    ...
    conn = _StubConn([_combo(2)], 1000, {"cnt": 10, "total": 1000})
    assert _run(conn) == []


# ── 급등이 아니면 잡지 않는다 ─────────────────────────────────────────
def test_flat_not_flagged():
    conn = _StubConn([_combo(50)], 1000, {"cnt": 50, "total": 1000})
    assert _run(conn) == []


def test_decline_not_flagged():
    conn = _StubConn([_combo(20)], 1000, {"cnt": 100, "total": 1000})
    assert _run(conn) == []


# ── baseline 이 없으면(모수 0) 평가하지 않는다 ────────────────────────
def test_no_baseline_skipped():
    conn = _StubConn([_combo(60)], 1000, {"cnt": 0, "total": 0})
    assert _run(conn) == []


def test_baseline_too_small_skipped():
    """baseline 표본이 하한 미만이면 비교하지 않는다 (±1건이 결과를 뒤집던 결함)."""
    conn = _StubConn([_combo(60)], 1000, {"cnt": 1, "total": 58})
    assert _run(conn) == []


def test_stale_incident_suppressed():
    """최근 구간 활동이 없으면(종료된 사건) 알리지 않는다."""
    conn = _StubConn([_combo(60, tail=0)], 1000, {"cnt": 10, "total": 1000})
    assert _run(conn) == []
...
def test_severity_escalation(defect_sev, expected):
    conn = _StubConn([_combo(60, severity=defect_sev)], 1000,
                     {"cnt": 10, "total": 1000})
    out = _run(conn)
...
def test_operator_threshold_respected():
    conn = _StubConn([_combo(30)], 1000, {"cnt": 10, "total": 1000})  # 3배
...
def test_zero_baseline_is_bounded():
    conn = _StubConn([_combo(60)], 1000, {"cnt": 0, "total": 500})
    out = _run(conn)
